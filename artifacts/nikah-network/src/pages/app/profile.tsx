...
function SectionEditor({ section, profile, onSave, onClose }: SectionEditorProps) {
  ...
  const inputClass = 'w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#10B981]/30 focus:border-[#10B981] transition-colors';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-end gap-3 p-5 border-t border-gray-100">
          <button onClick={onClose} className="px-4 py-2.5 border border-gray-200 text-gray-600 rounded-xl text-sm font-bold hover:bg-gray-50 transition-colors">Cancel</button>
          <button onClick={handleSave} disabled={saving || Object.keys(draft).length === 0}
            className="px-5 py-2.5 bg-[#10B981] text-white rounded-xl text-sm font-bold hover:bg-[#059669] disabled:opacity-50 transition-colors">
...
export default function AppProfile() {
  ...
  const completionColor = completion >= 100 ? 'bg-[#10B981]' : completion >= 60 ? 'bg-[#D97706]' : 'bg-red-400';
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5 pb-12">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        {/* Top gradient accent */}
        <div className="h-2 bg-linear-to-r from-[#10B981] via-emerald-400 to-[#D97706]" />
          ...
          <div className="flex items-start gap-5">
            ...
            <div className="relative shrink-0">
              ...
              <button
                ...
                className="absolute -bottom-1 -right-1 w-8 h-8 bg-[#10B981] text-white rounded-full flex items-center justify-center shadow hover:bg-[#059669] transition-colors disabled:opacity-60"
                title={checking ? 'Verifying face…' : 'Change profile photo'}
              >
                {checking ? <Search className="w-3.5 h-3.5" /> : photoLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Pencil className="w-3.5 h-3.5" />}
              </button>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                ...
                {profile.paymentStatus === 'completed' && (
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 text-xs font-semibold rounded-full border border-sky-100 text-sky-600 bg-sky-50">
      ...
      <ProfileSection title="Personal Details" icon="👤" onEdit={() => setEditSection('personal')}>
        {profile.bio && (
          <div className="sm:col-span-2 bg-[#F4F6F5] rounded-xl px-4 py-3 text-sm text-gray-600 italic border-l-2 border-[#10B981]">
            "{profile.bio}"
          </div>
        )}
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-50">
        <h2 className="font-bold text-[#1C1917] flex items-center gap-2.5">
          <span className="w-8 h-8 rounded-xl bg-emerald-50 flex items-center justify-center text-base">{icon}</span>
          <span>{title}</span>
        </h2>
        <button
          onClick={onEdit}
          className="text-xs font-bold text-[#10B981] hover:text-white border border-[#10B981] hover:bg-[#10B981] px-4 py-1.5 rounded-lg transition-colors"
        >
          Edit
        </button>
      </div>
